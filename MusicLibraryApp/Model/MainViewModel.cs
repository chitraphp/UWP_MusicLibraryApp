...
namespace MusicLibraryApp.Model
{
    class MainViewModel
    {
        ...
        public ObservableCollection<Song> currentPlayListSongs;
        ...
        {
            var str = song.ShortArtistName;
            foreach(var list in playLists)
            {
                if (list.PlayListName.Equals(p.PlayListName))
                {
                    list.PlayListSongs.Add(song);
                }
            }
           
        }

        public void GetplayListSongs(PlayList p)
        {
            foreach(var List in playLists)
            {
                if (List.PlayListName.Equals(p.PlayListName))
                {
                    foreach (var x in List.PlayListSongs)
                        this.currentPlayListSongs.Add(x);
                }
            }
            //this.currentPlayListSongs = p.PlayListSongs;
        }
